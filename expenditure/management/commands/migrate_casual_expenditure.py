...
from expenditure.models import ProvisionaryExpenditure


class Command(BaseCommand):
    help = 'Migrate casual expenditure data with duplicate phone number handling'
    ...
    def load_mappings(self):
        ...
        employee_pk_mapping_file_path = os.path.join(os.path.dirname(__file__), 'employee_log_file_casual.csv')
        # Load PK mappings for foreign keys
        project_pk_mappings = {}
        employee_pk_mappings = {}
        with open(project_pk_mapping_file_path) as f:
            reader = csv.DictReader(f)
            for row in reader:
                project_pk_mappings[row['old_pk']] = row['new_pk']

        with open(employee_pk_mapping_file_path) as f:
            reader = csv.DictReader(f)
            for row in reader:
                employee_pk_mappings[row['old_pk']] = row['new_pk']
        
        return project_pk_mappings, employee_pk_mappings

    def handle(self, *args, **options):
        project_pk_mappings, employee_pk_mappings = self.load_mappings()
        json_file_path = os.path.join(os.path.dirname(__file__), 'expenditure_casual.json')
        log_file_path = os.path.join(os.path.dirname(__file__), 'expenditure_casual_log_file.csv')
        ...
        with open(log_file_path, 'w', newline='') as csvfile:
            ...
            FIELD_MAPPING = {
                ...
                "conveyance" :"coveyance",
                "boat_fee": "boat",
                "fooding_fee" :"fooding",
                "hotel_fee": "hotel",
                "night_allownce" : "night_allowce",
                "others": "others",
            ...
            for exp_data in exps_data:
                try:
                    ...
                    # Create new project
                    exp = ProvisionaryExpenditure()
                    for target_field, source_field in FIELD_MAPPING.items():
                        if source_field is 'month':
                    ...
                    for target_field, source_field in FOREIGN_KEY_VALUES.items():
                        ...
                        if old_fk:
                            ...
                            if model_name == 'p_id':
                                new_fk = employee_pk_mappings.get(str(old_fk))
                                
                                value = Employee.objects.get(pk=new_fk)
                            elif model_name == 'job':
